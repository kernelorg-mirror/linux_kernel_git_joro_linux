...
static enum es_result ioio_exitinfo(struct es_em_ctxt *ctxt,
				    u64 *exitinfo)
{
...
handle_ioio(struct ghcb *ghcb, struct es_em_ctxt *ctxt)
{
	...
	if (exit_info_1 & IOIO_TYPE_STR) {
		...
		/*
		 * For the string variants with rep prefix we limit the amount
		 * of in/out operations per #VC exception so that the kernel has
		 * a chance to take interrupts an re-schedule while we emulate
		 * the instruction.
		 */
		...
		if (exit_info_1 & IOIO_TYPE_IN) {
			if (!ghcb_valid_rax(ghcb))
				return ES_VMM_ERROR;
			regs->ax = copy_lower_bits(regs->ax, ghcb->save.rax, bits);
		}
	}
...
static u64 insn_rmdata(struct es_em_ctxt *ctxt)
{
	u64 effective_addr;
	u8 mod, rm;
	...
	if (ctxt->insn.rex_prefix.nbytes && X86_REX_B(ctxt->insn.rex_prefix.value))
		rm |= 0x8;
	...
	if (ctxt->insn.sib.nbytes) {
		...
		if (index != 4)
			effective_addr += (*register_from_idx(ctxt->regs, index) << scale);
...
static unsigned long *insn_regdata(struct es_em_ctxt *ctxt)
{
	u8 reg;
	...
	if (ctxt->insn.rex_prefix.nbytes && X86_REX_R(ctxt->insn.rex_prefix.value))
		reg |= 0x8;
