...
static u64 sev_boot_info;

/*
 * Needs to be in the .data section because we need it NULL before bss is
 * cleared
 */
struct ghcb __initdata *boot_ghcb = NULL;
...
{
	/*
	 * This function runs on the first #VC exception after the
	 * kernel switched to virtual addresses.
	 */

	/*
	 * First save away the content of the GHCB MSR - it contains the
	 * location of the SMP jumptable from the BIOS.
	 */
	sev_boot_info = read_ghcb_msr();


	/* First make sure the hypervisor talks the same protocol as we do. */
...
dotraplinkage void do_vmm_communication(struct pt_regs *regs, long exit_code)
{
